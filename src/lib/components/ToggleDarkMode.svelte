<script lang="ts">
  ...
  const toggleTheme = () => {
    ...
    if (stored) {
      ...
      localStorage.setItem(storageKey, prefersDarkTheme() ? 'light' : 'dark')
    }
  ...
  const applyTheme = () => {
    const preferedTheme = prefersDarkTheme() ? 'dark' : 'light';
    currentTheme = localStorage.getItem(storageKey) ?? preferedTheme;
